			...
			<view class="slide-box" v-for="(item, index) in listData" :key="index">
				...
				>
					<view class="now-message-info" hover-class="uni-item--hover" :style="{ width: windowWidth + 'px' }" @click="clickItemMethod(item)">
						<image class="icon-image" :src="cdnUrl+item.collect_data_icon"  v-if="item.goods_status==2"></image>
						<view class="img" v-else>
							<image class="icon-image" :src="cdnUrl+item.collect_data_icon"></image>
							<image class="layer" src="../../static/xiajia.png"></image>
						</view>
						<view class="list-right">
							<view class="list-title" v-if="item.goods_name"><text v-if="item.activity_type=='5' || item.activity_type=='6'" style="font-size: 26rpx;padding: 5rpx;color:#fff;background: #fd484f;margin:0 5rpx;" v-html="item.activity_type=='5'?'拼团':'秒杀'"></text>{{ item.goods_name }}</view>
							<view class="list-detail" style="color: #FF3F3F; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" v-if="item.goods_type==0||item.goods_type==2">
							{{item.goods_cost!=0?'￥'+item.goods_cost/100:''}}
							</view>
							<view class="list-detail" style="color: #FF3F3F; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;margin-top: 5rpx;" v-if="item.goods_type!=0" >
							{{item.goods_type==2&&item.goods_cost!=0?'或':''}}{{item.goods_score_cost!=0?'￥'+item.goods_score_cost/100:''}}{{item.goods_integral&&item.goods_score_cost!=0?'+':''}}{{item.goods_integral!=0?item.goods_integral/100+'积分':''}}</view>
						</view>
					</view>
