<template>
	<view style="background: #F5F5F5;">
		...
		<view class="content" v-for="(item, index) in list" style="background-color: #FFFFFF;margin-bottom: 20rpx;padding-bottom:10rpx;">
			...
			<view class="btnList">
				...
				<view type="default" v-if="item.activity_type == 5 || item.activity_type == 6" style="border: 1px solid #CCCCCC;color: #999999;">标为特价</view>
				<view type="default" v-else @click="bwtj(item.goods_index, item.activity_type)">{{ item.activity_type == 0 ? '标为特价' : '取消特价' }}</view>
				<view type="default" @click="delGood(item.goods_index)">删除</view>
			</view>
		</view>
		<view v-if='list.length==0' style="text-align: center;background-color: #FFFFFF;">
...
export default {
	...
	methods: {
		...
		qrs(){
			if(parseFloat(this.nums2)<parseFloat(this.nums)){
				uni.showToast({
