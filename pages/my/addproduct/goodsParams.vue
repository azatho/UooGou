		...
		<view class="content">
			...
			<view class="iptContent" v-for="(item,index) in goods_params" :key="index">
				<view class="" style="width: 50%;">
					<input type="text" data-attr="name" @input="getInputVal($event,index)" :value="item.name" placeholder="必填" style="font-size: 22rpx;"/>
				</view>
				<view style="display: flex;align-items: center;width: 50%;">
                     <input type="text" data-attr='val' @input="getInputVal($event,index)" :value="item.val" placeholder="必填" style="font-size: 22rpx;"/>
					 <image src="../../../static/delParams.png" @click="delInput(index)" mode="" style="width: 36rpx;height: 36rpx;margin:30rpx 0 0 20rpx;"></image>
				</view>
			</view>   
			<image @click="addParams" src="../../../static/addcs.png" mode="" style="width: 216rpx;height:60rpx;display: block;margin-top: 30rpx;"></image>
		</view>  
...
export default {
	...
	onLoad(e) {
		if(e.goods_params){
			console.log(e.goods_params);
		   this.goods_params = JSON.parse(e.goods_params) || [];
		}
	},
	methods: {   
		getInputVal(e,index){
		...
		confime(){
			...
			page.$vm.formData.goods_params = JSON.stringify(this.goods_params);
			uni.navigateBack({
				data:1
			})
		}
