<template>
	<view
		style="background:#F5F5F5;padding: 10rpx 0rpx;font-size: 26rpx;color: #999999;font-family: PingFang SC;font-weight: 400;">
		...
		<!-- 商品主图 -->
		<view class=""
			style="margin-top:20rpx;background: #fff;display: flex;justify-content: space-between;align-items: center;padding: 10rpx 30rpx;">
			<view class="">
				<h2 style="color: #222222;font-size: 26rpx;font-family: PingFang SC;font-weight: 400;">商品主图</h2>
				<p style="margin-top:10rpx;font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">
					1.建议比例1:1</p>
				<p style="font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">
					2.为保证质量，单张大小不小于500KB</p>
			</view>
			<view class="" style="width: 120rpx;height: 120rpx;position: relative;">
				<image @click="upimg" :src="!formData.goods_icon ? uploadImg : $cdnUrl + formData.goods_icon"
					style="width: 120rpx;height: 120rpx;" mode="aspectFill"></image>
				<image @click="delGoods_icon" v-if="formData.goods_icon" src="../../../static/caca.png" mode=""
					style="width:36rpx;height:36rpx;position: absolute;right: -14rpx;top:-14rpx;"></image>
			</view>
		</view>
		<!-- 上传视频  -->
		<view class=""
			...
			<view class="">
				...
				<p style="margin-top:10rpx;font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">
					1.建议比例16:9</p>
				<p style="font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">2.为保证质量，建议大小为20MB
				</p>
			</view>
			<view class="" style="width: 120rpx;height: 120rpx;position: relative;">
				<image v-if="!formData.goods_video" @click="upvideo" :src="uploadVideo"
					style="width: 120rpx;height: 120rpx;" mode="aspectFill"></image>
				<video v-if="formData.goods_video" :src="$cdnUrl + formData.goods_video"
					style="width: 120rpx;height: 120rpx;" controls></video>
				<image @click="delGoods_video" v-if="formData.goods_video" src="../../../static/caca.png" mode=""
					style="width:36rpx;height:36rpx;position: absolute;right: -14rpx;top:-14rpx;"></image>
			</view>
		</view>
		<!-- 视频封面 -->
		<view class=""
			style="background: #fff;display: flex;justify-content: space-between;align-items: center;padding: 10rpx 30rpx;">
			<view class="">
			...
			<view class="" style="width: 120rpx;height: 120rpx;position: relative;">
				<image @click="upVideoImg"
					:src="!formData.goods_video_cover ? uploadImg : $cdnUrl + formData.goods_video_cover"
					style="width: 120rpx;height: 120rpx;" mode="aspectFill"></image>
				<image @click="delGoods_VideoImg" v-if="formData.goods_video_cover" src="../../../static/caca.png"
					mode="" style="width:36rpx;height:36rpx;position: absolute;right: -14rpx;top:-14rpx;"></image>
			</view>
		</view>
		<!-- 商品相册 -->
		<view class="" style="background: #fff;padding: 15rpx 30rpx;margin-top: 10rpx;">
			...
			<p style="margin-top:10rpx;font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">
				1.建议比例1:1,建议尺寸大小：800*800px</p>
			<p style="font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">2.为保证质量，单张大小不小于500KB
			</p>
			<view class="" style="display: flex;flex-wrap: wrap;margin-top: 20rpx;">
				<view class="" v-for="(item, index) in goods_img"
					style="margin-right:15rpx;width: 120rpx;height: 120rpx;position: relative;">
					<image :src="$cdnUrl + item" style="width: 120rpx;height: 120rpx;" mode="aspectFill"></image>
					<image @click="delGoodsList(index)" src="../../../static/caca.png" v-if="item" mode=""
						style="width:36rpx;height:36rpx;position: absolute;right: -14rpx;top:-14rpx;"></image>
				</view>
				<view class="" v-if="goods_img.length < 5">
					<image @click="upGoodsImgList" :src="uploadImg" style="width: 120rpx;height: 120rpx;"
	...
	<!-- 		<h2
				style="display: flex;align-items: center;font-size: 16rpx;font-weight: bold;color:#333;font-family: PingFang SC;padding: 15rpx 0;">
				金额设置
				<text style="color: #999999;">(至少填写一种，积分最少设置为100）</text>
			</h2> -->
			<!-- <checkbox-group @change="radioChange"> -->
				<view class="goodsName" style="display: flex;align-items: center;">
<!-- 					<view>
						<checkbox value="0" color="red"
							:checked="formData.goods_type == '0' || formData.goods_type == '2' ? true : false" />
						
					</view> -->
					<text style="color: #000000;">售价（元）</text>
					<input v-model="formData.goods_cost" type="text" value="" placeholder="请输入" />
				</view>
				<!-- <view class="goodsName" style="display: flex;align-items: center;">
					<view style="display: flex;align-items: center;">
						<checkbox value="1" color="red"
							:checked="formData.goods_type == '1' || formData.goods_type == '2' ? true : false" />
						<text>现金+积分</text>
					</view>
					<view class="" style="flex: 1;display: flex;">
						<text style="text-align: right;width: 200rpx;">现金</text>
						<input style="text-align:left;margin-left: 30rpx;" v-model="formData.goods_score_cost"
							type="text" value="" placeholder="请输入" />
						<text>积分</text>
						<input type="text" v-model="formData.goods_integral"
							style="text-align: left;margin-left: 30rpx;" value="" placeholder="请输入" />
					</view>
				</view> -->
			<!-- </checkbox-group> -->
			<view class="goodsName" style="display: flex;align-items: center;">
			...
			<!-- 属性图片 -->
			<view class=""
				style="margin-top:20rpx;background: #fff;display: flex;justify-content: space-between;align-items: center;padding-bottom: 20rpx;">
				<view class="">
					<h2 style="color: #222222;font-size: 26rpx;font-family: PingFang SC;font-weight: 400;">属性图片</h2>
					<p
						style="margin-top:10rpx;font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">
						1.建议比例1:1</p>
					<p style="font-size: 24rpx;font-family: PingFang SC;font-weight: 400;color: #999999;">
						2.为保证质量，单张大小不小于500KB</p>
				</view>
				<view class="" style="width: 120rpx;height: 120rpx;position: relative;">
					<image @click="upAttributeimg" :src="!formData.sku_pic ? uploadImg : $cdnUrl + formData.sku_pic"
						style="width: 120rpx;height: 120rpx;" mode="aspectFill"></image>
					<image @click="delAttributeimg" v-if="formData.sku_pic" src="../../../static/caca.png" mode=""
						style="width:36rpx;height:36rpx;position: absolute;right: -14rpx;top:-14rpx;"></image>
				</view>
			</view>
		</view>
		...
		<view class="" style="background: #fff;padding: 15rpx 30rpx;margin-top: 10rpx;">
			...
			<view class="" style="display: flex;flex-wrap: wrap;margin-top: 20rpx;">
				<view class="" v-for="(item, index) in goods_desc"
					style="margin-right:15rpx;width: 120rpx;height: 120rpx;position: relative;">
					<image :src="$cdnUrl + item" style="width: 120rpx;height: 120rpx;" mode="aspectFill"></image>
					<image @click="delGoodsDescList(index)" src="../../../static/caca.png" v-if="item" mode=""
						style="width:36rpx;height:36rpx;position: absolute;right: -14rpx;top:-14rpx;"></image>
				</view>
				<view class="" v-if="goods_desc.length < 5">
					<image @click="upGoods_descList" :src="uploadImg" style="width: 120rpx;height: 120rpx;"
						mode="aspectFill"></image>
				</view>
			</view>
		</view>
		<view style="padding: 5rpx 30rpx;">参数管理</view>
		<view @click="selectParams"
			style="display: flex;align-items: center;justify-content: space-between;padding: 20rpx 30rpx;background-color: #fff;">
			<view>
				<p
					style="color: #333333;font-weight: 400;font-family: PingFang SC;font-size: 26rpx;font-family: PingFang SC;">
					商品参数(选填)</p>
				<p>商品包装、尺寸、储存方式、保质期等</p>
			</view>
			<image src="../../../static/leftarrow.png" mode="" style="width: 17rpx;height: 32rpx;"></image>
		</view>
		<view @click="addGoodsSub" 
	...
	export default {
		...
		methods: {
			...
			upimg() {
				...
				uni.chooseImage({
					success: chooseImageRes => {
						...
						uni.uploadFile({
							url: self.$baseUrl +
							'/ugo/api/mall.php?c=uploadCheckImage', //仅为示例，非真实的接口地址
				...
				uni.chooseImage({
					success: chooseImageRes => {
						...
						uni.uploadFile({
							...
							success: uploadFileRes => {
								uni.hideLoading();
								self.formData.goods_video_cover = JSON.parse(uploadFileRes.data)
									.data;
							}
						});
					}
			...
			addGoodsSub() {
				...
				//  else if (this.formData.goods_type == '1' && (this.formData.goods_score_cost == '' || this.formData
				// 		.goods_integral == '')) {
				// 	return this.holo.toast({
				// 		title: '现金和积分都不能为空'
				// 	});
