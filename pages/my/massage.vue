					...
					<view class="msg" >
						<view class="tit">
							<view class="" style="flex:1">
								{{item.message_type==1?'发货提醒':(item.message_type==2)?'配送中':(item.message_type==3)?'配送完成':(item.message_type==4)?'退款失败':(item.message_type==5)?'您有一个新订单':(item.message_type==6)?'退货退款申请':item.message_type=='10'?'拼团消息':''}}
							</view>
							<view class="time">
								{{item.message_time}}
							</view>
						...
						<view class="con" @click="goDetail()">
							<view class="img">
								<image :src="cdnUrl+item.supplier_banner" mode=""></image>
							</view>
							<view class="right">
