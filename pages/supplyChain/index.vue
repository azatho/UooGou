	...
	<view class="index">
		...
		<view class="nearShops" v-if="stores.length != 0">
			<view class="title" @click="goNear">
				<view class="word">
					<image src="/static/nearAddr.png" mode=""></image>附近供应商
				</view>
		...
		<view class="recommend" v-if="goodsList.length != 0">
            ...
            ">
                      ...
                      items.is_have == 0 ? "领取" : "已领取"
							...
							<view class="price">
								￥{{ item.goods_cost / 100
                }}<text v-if="item.goods_cost != item.goods_price">{{
	...
	export default {
		...
		methods: {
			...
			init() {
				let self = this;
				self.request({
						url: "ugo/api/app.php?c=account/appIndexV2",
						data: {
							user_lng: self.lng,
							user_lat: self.lat,
							page:self.page,
			...
			getAreaSupplier() {
				...
				self
					.request({
						url: "ugo/api/app.php?c=getAreaSupplierV2",
						data: {
							user_lng: self.lng,
							user_lat: self.lat,
						},
			...
			goSearch() {
				uni.navigateTo({
					url: "/pages/index/search?lat=" + this.lat + "&lng=" + this.lng+'&type=1',
				});
			},
			//打开分类弹窗
			getClass() {
			...
			goClassify(name, id) {
				...
				uni.navigateTo({
					url: "/pages/index/classifyShop1?name=" +
						...
						this.lng +
						"&store_class=" +
						this.store_class+'&type=1'
				});
			},
			//附近门店更多入口
			goNear(e) {
				uni.navigateTo({
					url: "/pages/supplyChain/nearSupplyShop?lat=" + this.lat + "&lng=" + this.lng,
				});
			},
			//商品详情页
			goGoodsDetail(e) {
			...
			goshopDetail(e, goods_sort, goods_index) {
				// e后边是我添加的
				uni.navigateTo({
					url: "/pages/common/shopHome?id="+e+"&goods_sort="+goods_sort+"&goods_index="+goods_index+'&supplierID=0'
				});
			},
			//4月26
			openTips(){
