...
<template>
	<view class="goods">
				...
				<swiper class="banner_img" :autoplay="autoplay" :interval="interval" :duration="duration" circular="true">
					<swiper-item v-if="goods_info.goods_video">
						<video
							...
							@play="play"
							@pause="stop"
							:src="cdnUrl + goods_info.goods_video"
							:poster="cdnUrl + goods_info.goods_video_cover"
							object-fit="fill"
							:controls="controls"
							:duration="videoOften"
							x5-playsinline="true"
		...
		<view class="goodsTitle">
			...
			<view class="price">
				<view v-if ='goods_info.goods_type==0'>
					<span class='txt'></span>
					<text class="txt1">{{'￥'+$returnFloat(goods_info.goods_cost)}}</text>
				</view>
				<view v-if ='goods_info.goods_type==1'>
					<!-- <span class='txt'></span> -->
					<text class="txt1" v-if='goods_info.goods_score_cost!=0'>{{'￥'+$returnFloat(goods_info.goods_score_cost)+'+'}}</text>
					<text class="txt1" v-if='goods_info.goods_integral!=0'><img src="../../static/jifen.png" alt="" style='width: 36rpx;height: 36rpx;'> {{+$returnFloat(goods_info.goods_integral)+'积分'}} </text>
				</view>
<!-- 				<view v-if ='goods_info.goods_type==2'>
					<span class='txt'>￥</span>
					<text class="txt1">{{$returnFloat(goods_info.goods_cost)}}</text>
					<text class="txt2" v-if="goods_info.goods_cost != goods_info.goods_price">￥{{ goods_info.goods_price / 100 ? goods_info.goods_price / 100 : '0' }}</text>
					<view>
						<span class='txt'>￥</span>
						<text class="txt1">{{$returnFloat(goods_info.goods_score_cost)}}</text>或
						<text class="txt1">+<img src="../../static/jifen.png" alt="" style='width: 36rpx;height: 36rpx;'> {{$returnFloat(goods_info.goods_integral)}} 积分</text>
					</view>
				</view> -->
				<!-- <text class="txt1">{{ goods_info.goods_cost / 100 ? goods_info.goods_cost / 100 : '0' }}</text> -->
		...
		<view class="base_msg">
			<view class="choose" @click="getParameter" v-if="goods_info.goods_text">
				...
				<view class="right_img"><image src="../../static/back.png"><image src="../../static/back.png"></image></view>
			</view>
		...
		<view class="commonBgc"></view>
		<view>
			<view class="goods_imgs" v-for="(item, i) in goods_info.goods_introduce" :key="item"><image :src="cdnUrl + item" mode="widthFix"></image></view>
		</view>
		...
		<view class="addGoods">
			...
			<view class="" v-else>
				<view>
					<view class="goConfirmOrder" @click="getSpacs(0)">立即兑换</view>
				</view>
			</view>
		</view>
		...
		<view class="common" v-if="specsModel">
			<view class="common_con1">
				...
				<view class="spacs_img">
					<view class="img"><image :src="cdnUrl + sku_pic" mode=""></image></view>
					<view class="">
						<view class="price" v-if='goods_info.goods_type==0'>
							￥
							<text>{{goods_price}}</text><!-- +<text>{{$returnFloat(goods_info.goods_sku_price[0].sku_inventory)}}积分</text> -->
						</view>
						<view class="price" v-if='goods_info.goods_type==1'>
							<text v-if='goods_integral_price!=0'>{{'￥'+ goods_integral_price +'+'}}</text>
							<img src="../../static/jifen.png" alt="积分图标" style="width: 36rpx;height: 36rpx;">
							<text>{{goods_integral}}</text>积分
						</view>
	<!-- 					<view class="price" v-if='goods_info.goods_type==2'>
							￥
							<text>{{ goods_price }}</text>
							<br>
							或							
							<text>{{ goods_integral_price }}</text>
							+
							<img src="../../static/jifen.png" alt="积分图标" style="width: 36rpx;height: 36rpx;">
							<text>{{goods_integral}}</text>积分
						</view> -->
						<view>
							<view class="choose">已选：{{ sku_spacs == '无' ? '无规格' : sku_spacs }}</view>
							<view class="rest">剩余库存：{{ sku_stock }}</view>
						</view>
					</view>
				</view>
				<view v-if="sku.length != 0">
					<view class="spacs_info" v-for="(item, i) in sku" :key="i">
						<view class="spacs_tit">{{ item.sku_name }}</view>
						<view class="options">
							<block v-for="(items, k) in item.list" :key="k">
								<view :class="items.flag ? 'option' : ''" @click="getPrice(item.sku_index, items.sku_index, i, k,items)">{{ items.sku_name }}</view>
							</block>
						</view>
					</view>
				</view>
				...
				<view class="sure_num">
					...
					<view class="sure" @click="sure_shop" v-if="sku_stock != 0">确定</view>
...
export default {
	data() {
		return {
			...
			goods_name:'',
			goods_album:'',
			select_skuindex:"",//所选择商品规格的SKU
			my_count:"",//该商品兑换过的数量
			goods_limits:"",//该商品限购的数量
		};
	},
	onReady(res) {
	...
	methods: {
		...
		init() {
			...
			self.request({
				...
				if (res.data.success) {
					...
					console.log(88888)
					console.log(self.sku_price)
					self.my_count=res.data.data.my_count;//该用户兑换过的数量
					self.goods_limits=res.data.data.goods_limits;//该商品限购数量
					// console.log(9999)
					// console.log(this.sku_price)
		...
		getSpacs(e) {
			...
			if (this.sku.length != 0) {
				...
				if (this.goods_info.goods_type==0){
					this.goods_price =this.$returnFloat(this.sku_price[0].sku_cost_price) ;
				}else if (this.goods_info.goods_type==1){
					this.goods_integral_price =this.$returnFloat(this.sku_price[0].sku_score_price) ;
					console.log(this.sku_price[0].sku_integral_price)
					this.goods_integral = this.$returnFloat(this.sku_price[0].sku_integral_price);
				}
			}
		},
		// 默认选中第一项
		...
		// 点击切换获取规格和库存
		getPrice(parent, child, pn, cn,itemInfo) {
			let self = this;
		...
		obtainSpecs() {
			for (var i = 0; i < this.sku_price.length; i++) {
				if (this.skuIndex_list.join(',') == this.sku_price[i].sku_info) {
					// console.log(2222)
					if (this.goods_info.goods_type==0){
						this.goods_price =this.$returnFloat(this.sku_price[i].sku_cost_price) ;
					}else if (this.goods_info.goods_type==1){
						this.goods_integral_price =this.$returnFloat(this.sku_price[i].sku_score_price) ;
						console.log(this.sku_price[i].sku_integral_price)
						this.goods_integral = this.$returnFloat(this.sku_price[i].sku_integral_price);
					}
		...
		//选择商品数量
		addCount() {
			if (this.goods_limits!=0){
				if (this.goods_count < this.sku_stock) {
					if ((this.goods_count+this.my_count)<this.goods_limits){
						this.goods_count++;
					}else{
						uni.showToast({
							icon: 'none',
							title: '超过限购数量，请重新选择数量~'
						});
					}
				} else {
					uni.showToast({
		...
		sure_shop() {
			...
			let skuInfoValue="";
			let a
			if(self.goods_info.goods_sku_price.length==1){
				self.select_skuindex=self.goods_info.goods_sku_price[0].sku_index
			}else{
				for (txt of self.sku){
					for (value of txt.list){
						if(value.flag==true)skuInfo.push(value.sku_parent+':'+value.sku_index) 
					}
				}
				for ( a of self.goods_info.goods_sku_price){
					if(a.sku_info==skuInfo.toString()){
						self.select_skuindex=a.sku_index;
						break
					}
				}
			}
			if (uni.getStorageSync('token')) {
				uni.navigateTo({
					url:"confirmOrder?goods_index="+self.goods_info.goods_index+'&sku_index='+self.select_skuindex+'&goods_count='+self.goods_count
				})
			} else {
