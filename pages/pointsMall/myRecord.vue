			...
			<view class="list" v-for="(item,i) in orderList" :key ="i">
				<view class="one">
					<view class="left">
						...
						<text style="color: #999999;">订单号：{{item.order_id}}</text>
				...
				<view class="two" v-for="(items,k) in orderList[i].goods_list" :key ="k"  @click="goparticulars(item.order_index,item.statusgoods)">
					...
					<view class="details">
						...
						<view class="xia">
							<view class="univalence" v-if='items.goods_type==1'>{{items.goods_cost!=0?'￥'+items.goods_cost/100+'+':''}}{{item.order_coupon!=0?item.order_coupon/100+'积分':''}}</view>
							<view class="univalence" v-if='items.goods_type==0'>{{items.goods_cost!=0?'￥'+items.goods_cost/100:''}}</view>
				...
				<view class="three">
					...
					<view class="btn">
						...
						<view class="btn1"  v-if="item.order_status==6 && item.goods_list[0].goods_status==2">已完成</view>
					</view>
				</view>
			</view>
		</view>
		...
		methods: {
			...
			getStatus(e,item){
				...
				// console.log(item.order_index);
					...
					uni.navigateTo({
						url:'payOrder?order_index='+item.order_index+'&price='+item.order_total_price+'&order_total_score='+item.order_coupon+'&send_score='+item.order_send_score
					})
					// console.log('支付');
					// console.log(item.order_index);
