...
<!-- 		<view class="time" v-else>
		...
		<view class="price">
			<view class="goodprice">
				...
				<view class="two">{{orderInfo.order_goods_price!=0?'￥'+orderInfo.order_goods_price/100:''}}{{orderInfo.order_score_price!=0&&orderInfo.order_goods_price!=0?'+':''}}{{orderInfo.order_score_price!=0?orderInfo.order_score_price/100+'积分':''}}</view>
			</view>
			<view class="goodprice">
			...
			<view class="goodprice">
				...
				<view class="two1">{{orderInfo.order_total_price!=0?'￥'+orderInfo.order_total_price/100:''}}{{orderInfo.order_total_price!=0&&orderInfo.order_total_price!=0?'+':''}}{{orderInfo.order_score_price!=0?orderInfo.order_score_price/100+'积分':''}}</view>
			</view>
		</view>
		<view style="width: 100%; height: 20rpx; background-color: #F5F5F5;"></view>
		<!-- 订单信息 -->
		...
		<view>
			<view style="position: relative;" v-if='orderInfo.order_status==5||orderInfo.order_status==6'>
				...
				:markers='covers'
				></map>
				<view style="position: absolute;left: 380rpx;top: 170rpx;">
					<img src="../../static/mapLabel.png" alt="手机号背景框" width='150rpx' height='60rpx'>
				</view>
				<view style="position: absolute;left: 420rpx;top: 200rpx;">
		...
		<view class="btn">
			...
			<view class="btn3" v-if="orderInfo.order_status==5 || orderInfo.order_status==4" @click="gowuliu(orderInfo.order_index)">
				查看配送信息
			</view>
			<view class="btn1" v-if="orderInfo.order_status==6&&goods_list[0].goods_status==2" >已完成</view>
		</view>
	</view>
</template>
		...
		methods: {
			init(){
				...
				self.request({
					...
					data:{
						...
						if(res.data.success){
							self.covers[0].latitude=res.data.data.supplier_lat-0.001
			...
			getStatus(e,order_index,order_type,order_total_price,user_coupon){
				...
				if(e==0){// 删除订单
					...
					uni.navigateTo({
						url:'payOrder?order_index='+order_index+'&price='+order_total_price+'&order_total_score='+user_coupon
					})
