						...
						<block v-for="(item, i) in list" :key="i">
							<view class="con" @click="goodsDetail(item.goods_index)">
								...
								<view class="right">
									...
									<view class="total_price" style="display: flex;align-items: center;">
										<view v-if='item.goods_score_cost!=0' style="">￥{{item.goods_score_cost/100}}+{{item.goods_integral/100+'积分'}}</view>
										<view v-if='item.goods_score_cost==0' style="color: #FF3F3F;font-size: 24rpx;margin-top: 10rpx;display: flex;align-items: center;"><img src="../../static/jifen.png" alt="" style='width: 30rpx;height: 30rpx;'>{{item.goods_integral/100+'积分'}}</view>
										<view class="btn" @click="goodsDetail(item.goods_index)">兑换</view>
									</view>
								</view>
								<!-- <view v-if='item.goods_type=='>{{item}}</view> -->
							</view>
						</block>
	...
	export default {
		...
		methods: {
			...
			init() {
				let self = this;
				self.request({
					url: 'ugo/api/app.php?c=account/getScoreClassList',
					data: {
