<template>
	<view class="">
		...
		<view class="msg">
			...
			<view v-if="jumpType=='car'">
				<view v-for="(item,i) in goods_list" :key="i" class="con" >
					...
					<view class="right">
						<view class="right_msg">
						<text class='tit-tip' v-if='item.activity_type==6'>秒杀</text>
						<text class='tit-tip' v-if='item.activity_type==7'>特价</text>
						{{item.goods_name}}
						...
						<view class="num">
							<view class="" v-if='goods_info.can_order==1'>
									<text class="text2" v-if="item.goods_type==0">{{item.goods_cost!=0?'￥'+item.goods_cost/100:''}}</text>
									<text class="text2" v-if="item.goods_type!=0">{{item.goods_score_price!=0?'￥'+item.goods_score_price/100:''}}</text>
									<text class="text2" v-if="item.goods_type!=0">{{item.goods_score_price!=0&&item.goods_integral!=0?'+':''}}{{item.goods_integral/100}}积分</text>
							</view>
		...
		<view class="base_msg" v-if="jumpType=='car'">
			...
			<view class="msg_con">
				<view class="">配送费</view>
				<view class="">￥{{goods_info.base_price/100||'0'}}</view>
			</view>
			...
			<view class="msg_con">
				...
				<view class="price">{{goods_info.pay_total_price!=0?'￥'+goods_info.pay_total_price/100:''}}
					<text class="price" v-if="goods_info.total_score!=0">{{goods_info.pay_total_price!=0&&goods_info.total_score!=0?'+':''}}{{goods_info.total_score/100}}积分</text>
				</view>
...
<!-- 
		...
		<view class="sumbit_bgc" v-if="jumpType=='car'">
			<view class="money">
				应付金额：<text>{{goods_info.pay_total_price!=0?'￥'+goods_info.pay_total_price/100:''}}</text>
				<text v-if="goods_info.total_score!=0">{{goods_info.pay_total_price!=0&&goods_info.total_score!=0?'+':''}}{{goods_info.total_score/100}}积分</text>
			</view>
		...
		<view class="sumbit_bgc" v-else>
			<view class="money">
				应付金额：<text>￥{{goods_info.order_total_price/100?goods_info.order_total_price/100:'0'}}</text>
	...
	export default {
		...
		methods: {
			...
			init1() {
				let self = this;
				self.request({
					url: 'ugo/api/app.php?c=cartOrder',
					data: {
			...
			wxPay(){
				...
				jweixin.chooseWXPay({
				  ...
				  success: function (res) {
					  uni.redirectTo({
						  // url:'?tuan_id='+self.tuan_id+'&lat='+uni.getStorageSync('lat')+'&lng='+uni.getStorageSync('lng')
			...
			carAdd() {
				...
				self.request({
					url: 'ugo/api/app.php?c=submitOrderFromCart',
					data: {
						cart_index: self.cart_index,
						order_remark: self.order_remark,
						user_address_id: self.address_id,
					...
					if (res.data.success) {
						...
						uni.redirectTo({
							url:'confirmPay?order_id='+order_id+'&order_index='+self.order_index+'&price='+self.price+'&order_total_score='+order_total_score+'&send_score='+order_send_score
						})
						// self.request({
						// 	url: 'ugo/api/app.php?c=rechargeOrderUgo',
						// 	data: {
