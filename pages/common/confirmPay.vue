<template>
	<view>
		...
		<view class="text4" v-if="order_send_score!=0">
		...
		<view class="line20" ></view>
		<view class="scorePay-box" v-if='price!=0'>
			<view class="left" >
...
<script>
	...
	export default {
		...
		methods: {
			...
			confirmPay(){
				let self = this
				if(parseInt(self.userinfo.score)<=parseInt(self.order_total_score)){
					uni.showToast({
						title:'您的积分不足，无法购买',
						icon:'none'
					})
				}else if(parseInt(self.price)==0){
					this.interveryPay()
				}else{
					this.carAdd();
				}
			...
			interveryPay(){
				let self =this;
				self.request({
					url:"ugo/api/app.php?c=pay/rechargeOrderScore",
					data:{
						order_index:self.order_index,//订单号
						order_score:self.order_total_score//订单需要支付的积分
					}
				}).then(res => {
				        if (res.data.success) {
							uni.navigateTo({
								url:"successPay?jumpType=car"+'&order_total_price='+self.price+'&order_total_score='+self.order_total_score+'&good_index='+self.order_index
							})
				        } else {
				                uni.showToast({
