	...
	<view class="goods">
		...
		<view class="goodsTitle">
			...
			<view class="price">
				...
				<!-- <view class="txt1"   >￥{{ goods_info.goods_cost / 100 ? goods_info.goods_cost / 100 : '0' }}</view>
				<view class="txt1"  v-if="goods_info.goods_score_cost>0">或￥{{ goods_info.goods_score_cost / 100 ? goods_info.goods_score_cost / 100:'暂无'}} <text v-if="goods_info.goods_integral!=0">+{{ goods_info.goods_integral / 100 }}积分</text> </view>
				...
				<view class="txt1" v-if="goods_info.goods_type==0">{{ '￥'+goods_info.goods_cost / 100}}</view>
				<view class="txt1" v-if="goods_info.goods_type==1">{{ goods_info.goods_score_cost!=0 ? '￥'+goods_info.goods_score_cost / 100:''}}
					<text v-if="goods_info.goods_integral!=0">{{goods_info.goods_score_cost!=0?'+'+goods_info.goods_integral/100:goods_info.goods_integral/100 }}积分</text> </view>
				<view class="txt1" v-if="goods_info.goods_type==2">￥{{ goods_info.goods_cost / 100 ? goods_info.goods_cost / 100:''}}</view>
				<view class="txt1" v-if="goods_info.goods_type==2">或{{ goods_info.goods_score_cost !=0 ?'￥' +goods_info.goods_score_cost / 100:''}}
					<text v-if="goods_info.goods_integral!=0">{{goods_info.goods_score_cost !=0?'+':''}}{{ goods_info.goods_integral / 100 }}积分</text> </view>
				<view class="txt2" v-if="goods_info.goods_cost != goods_info.goods_price">￥{{ goods_info.goods_price / 100 ? goods_info.goods_price / 100 : '' }}</view>



			</view>
			<view class="btns">
				<view class="collect" :style="goods_info.yet_collect == 0 ? 'border:1px solid #fd484f;color:#fd484f' : 'border:1px solid #aaa;color:#aaa;'"
				 @click="collect(goods_info.yet_collect)">
					<image v-if="goods_info.yet_collect == 0" src="../../static/collectStar.png" mode=""></image>
					{{ goods_info.yet_collect == 0 ? '收藏' : '已收藏' }}
				</view>
				<view class="addShop" @click="getSpacs(1)">加入购物车</view>
			</view>
		</view>
		...
		<view class="shopMsg">
			...
			<view class="right" @click="getPhone(supplier_info.frequent_phone)">
				<image src="../../static/tel.png"></image>
				联系商家
				<!-- <a href="tel:18860351223">联系客服</a> -->
			</view>
		</view>
		<view>
		...
		<view class="addGoods">
			...
			<view class="" v-else>
				...
				<view class="" v-else>
					<image src="../../static/addcar.png" mode="" class="carImg" @click="goCar">
					...
					<view class="word">
						...
						<text class="price1">￥{{ total_price / 100 }}</text>
					</view>
					<view class="account" @click="confirmOrder">去结算</view>
				</view>
			</view>
		</view>
		...
		<view class="common" v-if="specsModel">
			<view class="common_con1">
				...
				<view class="spacs_img">
					<view class="img">
						<image :src="cdnUrl + sku_pic" mode=""></image>
					</view>
					<view class="">
						...
						<view class="price" v-if='goods_info.goods_type==1'>
							<text v-if='scorePrice!=0'>{{'￥'+ scorePrice +'+'}}</text>
			...
			<!-- 			<view class="price">
							...
							<view class="">
								<text v-if="score>0">或￥{{scorePrice}}+{{score}}积分</text>
							</view>
					...
					<view class="spacs_info" v-for="(item, i) in sku" :key="i">
						...
						<view class="options">
							<block v-for="(items, k) in item.list" :key="k">
								<view :class="items.flag ? 'option' : ''" @click="getPrice(item.sku_index, items.sku_index, i, k)">{{ items.sku_name }}</view>
							</block>
						</view>
					</view>
				</view>
